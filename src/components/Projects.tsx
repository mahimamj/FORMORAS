...
export default function Projects() {
  ...
  return (
    <section id="projects" className="section-padding bg-cream border-t border-stone-200/80">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            ...
            <h2 className="font-serif text-3xl sm:text-5xl lg:text-6xl text-charcoal font-normal">
              Featured Global <br />
              <span className="italic text-champagne-dark">Contract Projects.</span>
            </h2>
          </div>
        ...
        {/* Filters */}
        <div className="flex items-center space-x-2 overflow-x-auto pb-4 mb-12 no-scrollbar border-b border-stone-200">
          {categories.map((cat) => (
            <button
              key={cat}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project) => (
            <motion.div
              ...
              className="group cursor-pointer bg-white rounded-luxury overflow-hidden border border-stone-200/80 hover:border-champagne/60 transition-all duration-500 shadow-sm hover:shadow-luxury-soft flex flex-col"
            >
              <div className="relative aspect-[16/10] overflow-hidden image-container-zoom">
                ...
                <span className="absolute top-4 left-4 text-[10px] uppercase tracking-[0.2em] px-3 py-1 rounded-full bg-white/90 backdrop-blur-md text-champagne-dark border border-stone-200 font-semibold shadow-sm">
                  {project.sector}
                </span>
              </div>
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center space-x-2 text-[11px] text-stone-500 font-mono mb-2 font-medium">
                    <MapPin className="w-3.5 h-3.5 text-champagne-dark" />
                    <span>{project.location}</span>
                  </div>
                  <h3 className="font-serif text-2xl text-charcoal font-medium group-hover:text-champagne-dark transition-colors">
                    {project.title}
                  </h3>
                </div>

                <div className="mt-6 pt-4 border-t border-stone-200 flex items-center justify-between text-xs text-stone-600 font-sans">
                  <span>Scope: <strong className="text-charcoal font-medium">{project.unitsInstalled}</strong></span>
                  <div className="w-8 h-8 rounded-full border border-champagne-dark/40 flex items-center justify-center text-champagne-dark group-hover:bg-charcoal group-hover:text-cream transition-colors">
        ...
        {activeCaseStudy && (
          ...
          >
            <motion.div
              ...
              className="bg-cream border border-stone-200 rounded-luxury-lg max-w-4xl w-full p-6 sm:p-10 relative shadow-2xl max-h-[90vh] overflow-y-auto"
            >
              <button
                onClick={() => setActiveCaseStudy(null)}
                className="absolute top-6 right-6 w-10 h-10 rounded-full bg-stone-200 text-charcoal flex items-center justify-center hover:bg-charcoal hover:text-cream transition-colors font-bold text-xs"
              >
                <X className="w-6 h-6" />
              </button>
              ...
              <h2 className="font-serif text-3xl sm:text-4xl text-charcoal font-medium mt-1">
                {activeCaseStudy.title}
              </h2>
              <p className="text-xs text-champagne-dark font-mono font-medium mt-1">
                {activeCaseStudy.location} • Architect: {activeCaseStudy.architect} • Completed {activeCaseStudy.year}
              </p>

              {/* Main Banner */}
              <div className="my-6 rounded-luxury overflow-hidden aspect-[16/9] border border-stone-200 shadow-sm">
                <img
              ...
              <div className="space-y-4">
                <h4 className="font-serif text-xl text-charcoal font-medium">Executive Project Overview</h4>
                <p className="text-xs md:text-sm text-stone-600 font-normal leading-relaxed">
                  {activeCaseStudy.description}
                </p>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-4 border-y border-stone-200 text-xs font-sans">
                  <div>
                    <span className="text-stone-500 block font-mono">Contract Scope:</span>
                    <span className="text-charcoal font-medium">{activeCaseStudy.contractScope}</span>
                  </div>
                  <div>
                    <span className="text-stone-500 block font-mono">Volume / Scale:</span>
                    <span className="text-champagne-dark font-semibold">{activeCaseStudy.unitsInstalled}</span>
                  </div>
                </div>

                {/* Additional Gallery */}
                <h4 className="font-serif text-lg text-charcoal font-medium pt-2">Installation Gallery</h4>
                <div className="grid grid-cols-2 gap-4">
                  {activeCaseStudy.gallery.map((img: string, idx: number) => (
                    <div key={idx} className="rounded-luxury overflow-hidden aspect-[4/3] border border-stone-200">
                ...
                <div className="pt-4 flex justify-end">
                  <button
                    ...
                    className="flex items-center space-x-2 bg-charcoal hover:bg-stone-800 text-cream font-semibold text-xs uppercase tracking-[0.2em] px-6 py-3 rounded-luxury transition-colors shadow-luxury-soft"
                  >
